...
def postprocess(specimen_dir, indir, ids, error_list, intensity_threshold, threshold=0.3, size_threshold=2000, 
    ...
    try:
        ...
        tif.imwrite(os.path.join(specimen_dir,'MAX_' + indir.split('/')[-1] + '_ch1.tif'),
                    np.max(stack, axis=0))
        
        # #Save other MIP views
        # tif.imwrite(os.path.join(specimen_dir,'MAX_xz_' + indir.split('/')[-1] + 'axis1_ch1.tif'),
        #             np.max(stack, axis=1))

        #Save other MIP views
        tif.imwrite(os.path.join(specimen_dir,'MAX_yz_' + indir.split('/')[-1] + '_ch1.tif'),
                    np.max(stack, axis=2))
        ...
        # save max intensity projection
        tif.imwrite(os.path.join(specimen_dir,'MAX_{}_ch2.tif'.format(indir_base_path)),
                    np.max(stack, axis=0))
        ...
        # save max intensity projection
        tif.imwrite(os.path.join(specimen_dir,'MAX_' + indir.split('/')[-1] + '_ch3.tif'),
                    np.max(stack, axis=0))
        ...
        for i, f in enumerate(filelist):
            ...
            tif.imwrite(os.path.join(ch5_dir,'%03d.tif'%i), img) # consecutive numbers
            mask = np.zeros((img.shape), dtype=np.uint8)
            mask[img2 > img3] = 2
            mask[img3 > img2] = 3
            tif.imwrite(os.path.join(mask_dir,'%03d.tif'%i), mask) # consecutive numbers
        # except:
        ...
        for i in range(num_parts):
            ...
            for i in range(stack.shape[0]):
                tif.imwrite(os.path.join(savedir,'%03d.tif'%(i+idx1)), stack[i,:,:])
...
def main(specimen_dir,specimen_id,intensity_threshold):
